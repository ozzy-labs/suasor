...
観点ごとに独立した worker（並列実行単位）を起動し、各 worker が対応する `perspectives/<axis>.md` を読み込んで JSON で findings を返す。並列実行できないホストでは観点ごとに直列で独立評価しても結果は同等。
...
worker は観点 MD の読み込みと JSON 出力のみで完結させる（他スキルの呼び出しはしない）。

deep モードの並列起動機構はホスト依存（Claude Code: `SKILL.claude-code.md` の「deep モードでの並列起動」）。並列実行機構を持たないアダプタでは quick にフォールバックする（adapter 別の挙動は `SKILL.<adapter>.md` で吸収）。
