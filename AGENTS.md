...
アシスタント 18 skill（Suasor 同梱・`suasor skills install`）の SSOT は `docs/skills/<name>/SKILL.md`（[ADR-0008]）。エコシステム共通 dev skill（drive / lint / commit 等）は `@ozzylabs/skills` 経由。
