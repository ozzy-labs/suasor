...
汎用 `local` connector を実装した（`src/connectors/local.ts`、registry に 1 行追加・auth 不要なので `web` 同様 secret なし）。決定どおりの形:

- **identity**: `local:<sha1(絶対パス)>` / **source_type**: `local_file`（実体＝パス基準。`box:file:<id>` とは別 source なので二重取り込みは設定での住み分け運用、§3）
- **本文**: `textExtensions`（既定テキスト系一式）かつ `maxBytes`（既定 1MB）以内のファイルは本文（ファイル名 + 内容）、それ以外は name-only（box parity）
- **差分検知**: connector 供給の fingerprint `sha256(mtime:size:contentHash)`。symlink は辿らず、読めない entry は warning で skip（pass は止めない）
- CLI `suasor local sync` / MCP `connector.sync`（HITL）は registry 経由で自動的に利用可能
