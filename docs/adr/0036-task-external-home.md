...
1. **確定タスクは単一の外部ホームへ起票（egress write）** — 対象は Tier1（GitHub Issues / Jira / Slack List）。ユーザーが設定する**ただ1つの「タスクホーム」**へ起票する。**既定の変更（乗り換え）は可能**だが、**per-task の行き先上書きは初期スコープに含めない**（既定では出さない。概念としては閉じず、将来 additive に「既定ホーム + 任意 override」へ拡張する余地を残す）。プロジェクト/文脈単位のルールも必要時に additive に後付けする。
   - **Tier1 内でも API 成熟度に段差がある**: GitHub Issues / Jira REST は create/update/transition API が成熟。**Slack Lists の write・チェック状態取得 API はカバレッジが要検証**。よって **GitHub Issues を先行実装**し、Slack List は**実装前に write/状態取得 API の実在性を spike で確認**、不足時は Tier1 から外す（前提リスクを明示）。
   - **GitHub Projects v2 を別 destination `github_projects` として追加（#314）**: Issues（REST）とは別物（GraphQL Projects v2・project item / draft issue・カスタム単一選択 Status フィールド）なので、`github` とは独立の destination + 専用 actuator にする。起票は **draft issue**（`addProjectV2DraftIssue`）、identity は `ghp:<projectId>:item:<itemId>`、complete / reopen は **Status フィールド更新**（`updateProjectV2ItemFieldValue`。field / option の node id は project 固有＝**config 駆動**、Jira カスタムワークフローと同型）。`comment` は draft issue では非対応。冪等は suasor 層（`published_external_id`）+ body marker の first-page 走査。
