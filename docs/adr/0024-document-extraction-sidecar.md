...
5. **config** — `[extraction]`（`backend` 既定 `disabled` / sidecar `baseUrl` 等）。disabled / 到達不能時は graceful degradation（name-only のまま・取り込み成功）。embedding の `signal` 方式に倣う。**入力と抽出テキストにそれぞれ上限**（`maxBytes` / `maxTextChars`）を設け、巨大 PDF が store/FTS を膨張させないようにする（当初は 1 つの knob だったが、測る量が違い相反するため [#529](https://github.com/ozzy-labs/suasor/issues/529) で分離した）。
