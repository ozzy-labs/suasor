...
1. **demand は既存 ingest から導出する（新規 fetch 経路を作らない）。** demand 行 = 取り込み済み `slack_message` のうち、(i) 本文に operator の `<@USERID>` mention を含む、または (ii) DM（channel id が `D` 始まり）に属するもの。Slack を追加で叩かない（read-only / local-first を維持）。
