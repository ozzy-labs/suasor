...
特定 source の本文から task / decision / reply_draft 候補を抽出する HITL write skill（[ADR-0004](https://github.com/ozzy-labs/suasor/blob/main/docs/adr/0004-mcp-agent-boundary-and-hitl.md)）。
...
read で本文を取り、write は HITL。**auto-apply 経路は存在しない**（[ADR-0004](https://github.com/ozzy-labs/suasor/blob/main/docs/adr/0004-mcp-agent-boundary-and-hitl.md)）。

1. 対象 source を特定する（`externalId` 不明なら `search` / `find` で確定）
2. `source.get`（`externalId`）で対象 source の本文を読む
3. `propose.generate`（mode=`source_extract`）で task / decision / reply_draft 候補を生成する（それぞれ `TaskProposed` / `DecisionRecorded` / `ReplyDraftProposed` 候補に対応、[data-model.md](https://github.com/ozzy-labs/suasor/blob/main/docs/design/data-model.md)）
