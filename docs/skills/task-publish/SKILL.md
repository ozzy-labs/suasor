...
3. 承認後:
   - 未起票の確定タスク → `task.publish`（任意 `destination` or `[tasks].default` の `[tasks.homes.<destination>]` へ起票。`TaskPublished` を append、外部 id を記録）
   - 起票済みタスク → `task.act`（`complete` / `reopen` / `comment`。`TaskActionIssued` を append）。actuator 設定はその task 自身の `published_destination` で解決される（R1-3）ので、既定を乗り換えても既存 published task の操作は壊れない
