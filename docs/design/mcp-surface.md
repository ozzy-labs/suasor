...
戻り値: `{ "source": {...} | null, "links": [{ kind, id, relation, direction, linkId? }], "extractionMeta": { version, state, updatedAt } | null }`。`links` は source を `from` とする outgoing edge のみ（`graph.related` と同形）。`extractionMeta` は抽出されていない source（プレーンテキスト connector 本文等）では `null`（[ADR-0024](../adr/0024-document-extraction.md)）。未知 id は `{ source: null, links: [], extractionMeta: null }`（エラーにしない）。副作用なし。
