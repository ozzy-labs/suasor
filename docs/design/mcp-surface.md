...
| `graph.related` / `graph.expand` | 既存 `links` projection 上の provenance traversal（[ADR-0018](../adr/0018-knowledge-graph-traversal.md)）。`graph.expand` の `direction` で後方トレース（[ADR-0020](../adr/0020-multi-actor-coordination-scope.md)、下記参照） | 実装済み（#71 / #97） |
...
既存 `links` projection 上の provenance traversal。`graph.related` は origin の 1-hop 隣接、`graph.expand` は depth/limit で束ねた BFS 展開を返す。relation は `derived_from` / `replies_to` / `references`。本文は `source.get` で取得する。

| tool | 引数 | 戻り値キー |
|---|---|---|
| `graph.related` | `kind` / `id` / `direction?: "out"\|"in"\|"both"`（既定 `both`） / `relation?` | `{ "origin", "neighbors": [{ kind, id, relation, direction }] }` |
