...
- 現状 **egress-free な `ollama`（ローカルサイドカー）のみ実装** で既定経路。`openai` / `voyage` は config 上受理するが **未実装の opt-in placeholder**（embedder は `null` ＝ FTS に degrade）で、実装時は外部 API への **egress を伴う**点が `ollama`（ローカル完結）と非対称。`openai` / `voyage` を設定すると黙って無効化されないよう **起動時（`suasor mcp serve`）と `suasor doctor` が WARN を出す**（silent-error 撲滅・[Issue #235](https://github.com/ozzy-labs/suasor/issues/235)）
