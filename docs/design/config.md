...
- secrets: OS keychain（@napi-rs/keyring）。env override 経路あり（headless/Docker 用）。keychain 実装は #10
...
- `[connectors.<name>]` は open record（foundation では値を緩く保持）
...
- secrets の env override（例: `SUASOR_CONNECTOR_<NAME>_TOKEN`）は #10 で確定
