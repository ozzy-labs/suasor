...
- 確定テーブル:
  ...
  - `commitments` — `id`(PK) / `title` / `direction`（owed_by_me / owed_to_me）/ `state`（open / resolved / dismissed）/ `due_date` / `person`（記録どおりの生文字列・表示用）/ `person_id`（fold 時に person identity graph へ解決した正規化人物・曖昧なら NULL、[#443](https://github.com/ozzy-labs/suasor/issues/443)）/ `created_at` / `updated_at`（commitment 台帳、[ADR-0021](../adr/0021-commitment-ledger.md) / #91。`commitment.list` が読む。`PersonsMerged` は `person_id` を cascade する）
  - `demand_seen` — `external_id`(PK = demand 行の source id) / `state`（acked / dismissed）/ `seen_at`（導出 demand の seen-state、[ADR-0041](../adr/0041-neutral-demand-priority-substrate.md) / #419。`DemandAcknowledged` / `DemandDismissed` を last-write-wins で畳む。`demand.list` が既定でこの印の付いた行を除外＝「未処理」を真にする。demand 自体は stored entity でなく `sources` からの導出 view なので seen だけを別に持つ）
  ...
  - `slack_channels` — `channel_id`(PK) / `team_id` / `name` / `kind`（public / private / group / dm）/ `observed_at`（Slack channel 名 projection、[ADR-0037](../adr/0037-slack-name-enrichment.md) / #354。sync 時に `conversations.info` 等で名前解決し last-write-wins で畳む。表示層はローカル join のみ＝no-fetch-at-query。`name` 空＝未解決で id フォールバック、後続 sync で enrich）
  - `slack_teams` — `team_id`(PK) / `name` / `observed_at`（Slack team / workspace 名 projection、[ADR-0037](../adr/0037-slack-name-enrichment.md) / #361。sync 時に `auth.teams.list`（Grid）/ `auth.test`（単一）で名前解決し last-write-wins で畳む。1 team = N channels なので channel と別 projection。表示層はローカル join のみ＝no-fetch-at-query。`name` 空＝未解決で id フォールバック、後続 sync で enrich。`demand.list` が `meta.team` で LEFT JOIN して `teamName` を返す）
