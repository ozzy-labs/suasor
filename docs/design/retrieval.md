...
- **search（query 時）**: `recall.search`（[mcp-surface](mcp-surface.md)）が query を埋め込み、`vec0` の KNN（`WHERE embedding MATCH ? AND k = ?`）で最近傍を引き、`sources` を JOIN してメタ + 本文（既定は先頭 N chars の excerpt、`fullBody` で全文・retrieval-m2）を返す。`score` は L2 distance（小さいほど近い、best-first）
