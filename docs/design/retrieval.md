...
- **ランキング**: SQLite `bm25(sources_fts)` 昇順（よりマイナス＝より関連、best-first）。`sources` を JOIN して `source_type` / `observed_at` / 本文（既定は excerpt、後述）を返す
- **短クエリ fallback**: trigram は 3-gram のため、**最長 token が 3 code point 未満**のクエリは MATCH で 0 件になる。その場合 `sources.body` に対する `LIKE` substring scan に fallback する。**各 token を個別の `LIKE '%token%'` 述語に分割し AND 連結**する（`%` `_` `\` をエスケープ、`ESCAPE '\'`）ため、複数短 token クエリ（例: 「予算 承認」）は「両 token を含む文書」を引く（従来は空白込みの連続部分文字列 `%予算 承認%` を 1 パターン検索し、空白を持たない日本語ではほぼ 0 件になっていた・retrieval-2）。ランキングは **body 中の全 token の出現回数合計**（`length`/`replace` で算出、`lower()` で ASCII は大文字小文字非依存＝LIKE と一貫・CJK は no-op）降順、同数は `observed_at` 降順。`score` はその出現回数（**FTS の bm25 とは向きが逆で、大きいほど関連**）。日本語の 1〜2 文字クエリ（例: 区・会議）もこの経路で拾える
...
- **メタフィルタ（任意・#142）**: `searchSources(sqlite, query, { sourceType?, observedAfter?, observedBefore?, limit?, fullBody?, maxBodyChars? })`。`sourceType` は `sources.source_type` 完全一致、`observedAfter` / `observedBefore` は `observed_at` の窓（**下限 inclusive `>=` / 上限 exclusive `<`**、projection 読み取り tool と同一規約）。フィルタは JOIN 済み `sources` 行に対する WHERE で、**FTS / LIKE fallback の両経路に同一適用**（ランキングは不変、候補集合を絞るのみ）。フィルタ未指定時は従来結果と一致（additive）
- **payload 抑制（[ADR-0018](mcp-surface.md) / retrieval-m2）**: 既定では全文 `body` ではなく**上限付き excerpt**（既定 `DEFAULT_EXCERPT_CHARS` = 240 code point）を返す。lexical hit（FTS / LIKE fallback）は**最初にマッチした token を中心**に window を切り出し（`…match…`）、それ以外（意味検索 recall）は**先頭 N chars**を切り出す。切れた側に `…` を付す。全文は `source.get` に委譲（複数 hit 応答で host context を溢れさせないため）。`fullBody: true` で全文 `body`、`maxBodyChars: N` で excerpt 長を上書き（`search` の全 mode 共通・破壊的変更）
- **戻り値**: `{ hits: SearchHit[], strategy: "fts" | "like-fallback", totalHits: number, truncated: boolean, analyzedQuery: string[] }`。`SearchHit = { externalId, sourceType, observedAt, score, excerpt?, body? }`（既定は `excerpt`・`fullBody` 指定時のみ `body`、いずれか片方）
- **透明性フィールド（ADR-0007「no silent wrong answer」）**: `totalHits` は `limit` 適用前の総マッチ数（`COUNT(*)`、ページが満杯のときのみ追加クエリ）で常に `>= hits.length`。`truncated` は `totalHits > hits.length`（`limit` で打ち切られたか）。`analyzedQuery` は実際に検索に使われたトークン列で、**FTS / LIKE fallback とも whitespace 分割トークン**（fallback も per-token AND 化したため trimmed query 1 要素ではなくなった）。エージェントが「20/20 打ち切り」と「5/5 完全」を区別し、痩せ/空結果の原因を把握できるようにする
...
- **search（query 時）**: `search`（`mode=semantic`・[mcp-surface](mcp-surface.md)）が query を埋め込み、`vec0` の KNN（`WHERE embedding MATCH ? AND k = ?`）で最近傍を引き、`sources` を JOIN してメタ + 本文（既定は先頭 N chars の excerpt、`fullBody` で全文・retrieval-m2）を返す。`score` は L2 distance（小さいほど近い、best-first）
- **メタフィルタ（任意・#142）**: 意味検索も `sourceType` / `observedAfter` / `observedBefore` を受ける。KNN は vec テーブルに `k = ?` を課す制約上、任意の述語を index に push できないため、**JOIN 済み `sources` 行への post-filter**で実装する: `limit * RECALL_FILTER_OVERFETCH`（既定 4 倍）の近傍を多めに引いてからフィルタし `limit` に trim する（フィルタで上位近傍が落ちても `limit` 件に届くようにするため）。時間窓の規約は FTS と同一（下限 inclusive / 上限 exclusive）

## Hybrid（FTS × 意味検索の RRF 融合・#142）

FTS-first（ADR-0005）を保ったままの additive 拡張。`search` read tool の `mode="hybrid"` が FTS と vec を**それぞれ走らせ、2 つのランク済みリストを Reciprocal Rank Fusion（RRF）で融合**する。lexical（FTS: 完全一致・キーワード）と semantic（vec: 言語跨ぎ・語彙ミスマッチ）の盲点を相互補完する。
