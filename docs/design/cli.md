...
実装状況: `init` / `db migrate` / `projections rebuild` / `search` / `mcp serve`（read tools・[ADR-0004](../adr/0004-mcp-agent-boundary-and-hitl.md)）は稼働。`<connector> sync` /
`skills install` / `skills list` は command surface のみ確定し、本体は後続 Issue
（connector=#7–#12 / skills=[ADR-0008](../adr/0008-assistant-skills.md)）。
