...
実装状況: `init` / `db migrate` / `projections rebuild` / `search` / `github sync` は稼働。
`mcp serve` / `skills install` / `skills list` は command surface のみ確定し、本体は後続 Issue
（MCP=[ADR-0004](../adr/0004-mcp-agent-boundary-and-hitl.md) / skills=[ADR-0008](../adr/0008-assistant-skills.md)）。
