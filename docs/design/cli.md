...
- 長時間コマンド（`<connector> sync` / `sync` / `projections rebuild` / `embeddings rebuild` / `embeddings drain` / discovery verb 群 / `slack conversations`）の TTY 進捗表示は実装済み（`src/cli/progress.ts` の `createProgress`・stderr / TTY 限定・`--no-progress` で無効化）。env 上書きは現状未導入（必要が出れば別途検討）
