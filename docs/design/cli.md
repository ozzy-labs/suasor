...
- `<connector> sync` 実行中は **stderr に進捗（処理件数）を表示**する（`src/cli/progress.ts`）。stdout / `--json` を汚さないよう stderr、かつ **TTY 限定**（CI / パイプ / リダイレクトでは自動的に無音）。`--no-progress` で明示無効化（opshub ADR-0026 相当）。`slack conversations` も同じ `createProgress` を使い、DM 名前解決ループ（`users.info`）と `--sort=last_self_post` の `search.messages` ページングを進捗表示でラップする（#84）。同様に、event 数 / source 数に比例して長くなる保守 verb も同じ indicator を使う: `projections rebuild`（replay の event ごとに tick）、`embeddings rebuild` / `embeddings drain`（embed したソースごとに tick）。後者 2 つは進捗を出せるよう maintenance 側を `MAINTENANCE_EMBED_BATCH`（128）単位の chunk ループに分割しており、これは全 body / 全 vector を一度に載せないピークメモリ抑制も兼ねる（chunk 途中でサイドカーが失敗しても先行 chunk の embed は保持し partial count を返す best-effort・[ADR-0006](../adr/0006-ml-delegation.md)）
