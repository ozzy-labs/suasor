...
- **対応 connector**: `local`（OS 同期フォルダ含む任意ディレクトリ）に加え、**`box` / `ms-graph`（OneDrive）が API 経由で本文 fetch → 抽出**に対応済み（[ADR-0034](../adr/0034-api-connector-extraction.md)）。同じサイドカー / `extraction_meta` を再利用し、connector 側で内容ベース fingerprint + lazy download を実装する。google(Drive) の API 経路も同じ共通基盤で段階展開予定（後続 Issue #242）
...
- カバレッジは `local_file` / `box_file` / `ms365_file`（OneDrive）の各 source type を横断して集計する（共通基盤、#241 / #243）
...
- `box` / `ms-graph`（OneDrive）の API 実体は本 connector で抽出対応済み（[ADR-0034](../adr/0034-api-connector-extraction.md)）。google(Drive) の API 実体は同じ共通基盤で段階展開中（後続 Issue #242）。`local`（OS 同期フォルダ）にあるものは `local` でもカバーできる
- スプレッドシート等は text 化で表構造が落ちる（まず text/Markdown 化・構造化抽出は将来）
- `maxBytes` 超過ファイルは name-only のまま（Box / OneDrive は `size` メタで fetch 前に判定）
