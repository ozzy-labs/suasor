...
- **対応 connector**: `local`（OS 同期フォルダ含む任意ディレクトリ）に加え、**`box` / `google`(Drive)（API 経由で本文 fetch → 抽出）** が対応済み（[ADR-0034](../adr/0034-api-connector-extraction.md)）。同じサイドカー / `extraction_meta` を再利用し、connector 側で内容ベース fingerprint + lazy download を実装する。ms-graph(OneDrive) の API 経路も同じ共通基盤で段階展開予定（後続 Issue #243）
...
- カバレッジは `local_file` / `box_file` / `google_drive` の各 source type を横断して集計する（共通基盤、#241 / #242）
...
- `box` / `google`(Drive) の API 実体は本 connector で抽出対応済み（[ADR-0034](../adr/0034-api-connector-extraction.md)）。ms-graph(OneDrive) の API 実体は同じ共通基盤で段階展開中（後続 Issue #243）。`local`（OS 同期フォルダ）にあるものは `local` でもカバーできる
- スプレッドシート等は text 化で表構造が落ちる（まず text/Markdown 化・構造化抽出は将来）。Google Sheets も export 経由で xlsx → text 化のため同様
- `maxBytes` 超過ファイルは name-only のまま（Box / Drive binary は `size` メタで fetch 前に判定。Google ネイティブは fetch 前サイズが取れず抽出後の text 上限のみ適用）
