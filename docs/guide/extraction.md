...
Word / Excel / PowerPoint / PDF は既定では**ファイル名のみ**取り込まれ、本文は検索・要約できない。`[extraction]` サイドカーを有効にすると、取り込み時にこれらの本文を text/Markdown 化して `search` / 意味検索 / `find` / `source-review` から参照できるようになる（[ADR-0024](../adr/0024-document-extraction-sidecar.md)）。
