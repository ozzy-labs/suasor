...
- **対応 connector**: `local`（OS 同期フォルダ含む任意ディレクトリ）に加え、**`box`（API 経由で本文 fetch → 抽出）** が対応済み。google-drive / ms-graph(OneDrive) の API 経路は同じ共通基盤で段階展開予定（後続 Issue #242 / #243）
...
- google-drive / ms-graph(OneDrive) の API 実体は同じ共通基盤で段階展開予定（後続 Issue #242 / #243）。`local`（OS 同期フォルダ）にあるものは `local` でもカバーできる
