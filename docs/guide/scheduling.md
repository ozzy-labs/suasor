...
`suasor sync` は**有効な connector を 1 回ずつ取り込む**短命・冪等な one-shot コマンド（[ADR-0027](../adr/0027-bulk-sync-orchestration.md)・FR-ING-5/6）。有効判定は `connectors list` / `doctor` と同じ（`[connectors.<name>]` slice が存在し `enabled = false` でない）。connector 間は **bounded pool で並列**に取り込む（既定並列度 4・各 connector は別 API ホスト＝独立 rate-limit のため。connector 内の per-resource は直列を維持）。コマンド・フラグの詳細は [CLI リファレンス](../design/cli.md) を参照。

```bash
suasor sync                                  # 有効 connector を一括取り込み（並列）
suasor sync --connector github,slack         # 対象を絞り込む
suasor sync --concurrency 2                  # 同時に sync する connector 数を 2 に制限
...
- **並列（connector 間）**: connector 間は既定並列度 4 の bounded pool で sync する（別 API ホスト＝独立 rate-limit のため。sync 時間の大半はネットワーク待ちなので利得が大きい）。`--concurrency N` で上書き（8 超は警告のみ）。**connector 内の per-resource は直列**（同一 API のクォータ共有を避ける）。`--no-continue-on-error`（fail-fast）時は直列で実行する
...
`suasor sync status` 自体は **専用の staleness exit code や `--max-age` フラグを持たない**（鮮度の閾値判定は運用者の責務）。`--json` は connector ごとに `{ connector, status, startedAt, endedAt, observed, updated, unchanged, durationMs, lastError }`（未 sync は `{ connector, status: "never_synced" }`）の配列を返すので、これを `jq` 等で処理して「最終 sync からの経過」を閾値判定できる。

```bash
# 直近 sync（endedAt）が 90 分より古い、または失敗 / 未 sync の connector を検出。
# 1 件でも該当すれば exit 1 にして cron / CI の gate に使う。
suasor sync status --json | jq -e --argjson maxAgeSec 5400 '
  (now) as $now
  | [ .[]
      | select(
          .status == "never_synced"
          or .status == "error"
          or (.endedAt == null)
          or (($now - (.endedAt | fromdateiso8601)) > $maxAgeSec)
        )
    ] as $stale
  | ($stale[] | "stale/failed: \(.connector) (\(.status))"), ($stale | length == 0)
' || echo "WARN: a connector sync is stale or failed"
```
