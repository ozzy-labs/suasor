...
- **注（filename-only ingest）**: 現状ファイル本文はダウンロードせず、取り込む `body` は**ファイル名のみ**。Box 取り込みはファイルを名前で検索可能にするが、ファイル内容のインデックスは行わない。fingerprint は body（= ファイル名）から導出するため、内容だけ変わってもファイル名が同じなら冗長な `SourceBodyUpdated` は発生しない（rename はファイル名 = body が変わるので更新として検知される）。本文抽出（API 経由の lazy download + `sha1` 内容 fingerprint + サイドカー抽出）は [ADR-0034](../adr/0034-api-connector-extraction.md) で設計済み・段階展開中（epic #232）
