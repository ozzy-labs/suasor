 ...
 * content-derived from the title + rationale + provenance (id.ts, matching the
 * `decision` candidate fingerprint), so re-recording the same decision is a
 * no-op; the result reports whether the event was appended (`created`) or the
 * decision already existed (`existing`). Since #435 the `rationale` is part of
 * the identity: recording the same title with a *different* rationale creates a
 * distinct decision instead of silently colliding with (and losing to) the
 * first recorded rationale.
 */
import { z } from "zod";
...
export function decisionRecord(
  ...
  // — the same fingerprint propose.apply uses for `decision` candidates, so the
  // human path and the model-suggested path share one identity for equal content.
